...
def seed(config_path: str, count: int) -> None:
    ...
    conn.commit()
    conn.close()
    print(f"Готово: вставлено {inserted}, пропущено {skipped} (уже существовали).")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Наполнить БД тестовыми заявками")
    parser.add_argument(
        "--count", type=int, default=500, help="Количество записей (по умолчанию 500)"
    )
